...
def main():
    ...
    with Manager() as manager:
        ...
        panP = manager.Value("f", 0.103) 
        panI = manager.Value("f", 0.073)
        panD = manager.Value("f", 0.004) # Try to keep these an order of magnitude lower than your P and I values

        # PID values for tilting
        tiltP = manager.Value("f", 0.103) 
        tiltI = manager.Value("f", 0.073)
        tiltD = manager.Value("f", 0.004) # Try to keep these an order of magnitude lower than your P and I values
